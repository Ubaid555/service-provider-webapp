  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let update = await fetch(
        `http://localhost:4500/updateBooking?userId=${bookingId}`,
        {
          method: "PUT",
          headers: {
  ...
  return (
    <>
      ...
      <h1 className={styles.main_heading}>
        Update {JSON.parse(localStorage.getItem("loginusers")).name}'s PROFILE
      </h1>
      <div className={styles.updateProfileContainer}>
        <form className={styles.cardInfo}>
          ...
          <div className={styles.formGroup}>
            <label htmlFor="name" className={styles.cardTitle}>
              Name
            </label>
            <input
              type="text"
