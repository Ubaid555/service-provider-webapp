...
const SeeReviews = () => {
    ...
    useEffect(() => {
        const fetchReviews = async () => {
            try {
                ...
                setReviews(data.filter(review => review.serviceProviderId === serviceProviderId && review.category === category));
            } catch (error) {
    ...
    return (
        <>
        <Navbar/>
            <h2 className={styles.seereviewHeading}>Reviews for {serviceProviderName}'s {category} services</h2>
            <div className={styles.reviewsContainer}>
                {reviews.length > 0 ? (
                    ...
                    <p className={styles.no_Found}>No reviews yet for this service provider and category</p>
                )}
            </div>
        <Footer/>
        </>
